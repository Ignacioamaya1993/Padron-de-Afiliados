...
const carpetaAfiliado = afiliado?.numero_afiliado
  ? `afiliados/${afiliado.numero_afiliado}/internaciones`
  : "afiliados/sin_numero/internaciones";
...
return `
  <div class="card" data-id="${inter.id}" data-index="${index}" data-lugar-id="${inter.lugar_internacion_id || ""}">
    ...
    <div><label>Fecha Ingreso</label>
      <input type="date" name="fecha_ingreso" readonly value="${fISO(inter.fecha_ingreso)}">
    </div>
  ...
  const { error } = await supabase
    ...
    .eq("id", id);
      ...
      for (const adj of (card._adjuntosNuevos || [])) {
        ...
        const url = await subirArchivoCloudinary(input.files[0], carpetaAfiliado);
